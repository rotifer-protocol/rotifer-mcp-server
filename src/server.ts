...
export function createServer(): Server {
  ...
  const allowedHatches = resolveAllowList();

  // Listed here rather than inside the handler for the same reason ALL_TOOLS is:
  // the list and the gate that filters it have to be readable side by side.
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            no_sandbox: { type: "boolean", description: "Force Node.js execution, skip WASM sandbox. Refused unless the server was launched with --allow=no-sandbox." },
          },
          required: ["agent_name"],
        },
      },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            no_sandbox: { type: "boolean", description: "Run without WASM sandbox, Node.js only (default: false). Refused unless the server was launched with --allow=no-sandbox." },
            trust_unsigned: { type: "boolean", description: "Allow Node.js execution for Cloud-installed genes (default: false). Refused unless the server was launched with --allow=trust-unsigned." },
          },
          required: ["gene_name"],
        },
      },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    // the sandbox. Unlike the set, this applies whether or not one is declared:
    // an escape hatch is a property of the option, not of who is asking.
    const blocked = blockedEscapeHatches(name, args as Record<string, unknown> | undefined, allowedHatches);
    if (blocked.length) {
      throw new McpError(ErrorCode.InvalidParams, escapeHatchMessage(blocked));
    }
    ...
    try {
      ...
      logMcpCall({
        ...
        success: false,
        latency_ms: Date.now() - startMs,
        caller: loadCredentials()?.user?.id ?? null,
      });
  ...
  // leaderboard to a caller that asked for none of them.
  server.setRequestHandler(ListResourceTemplatesRequestSchema, async () => ({
    resourceTemplates: ALL_RESOURCE_TEMPLATES.filter((r) =>
      resourceTemplateAllowed(r.uriTemplate, allowedTools)
    ),
  }));
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    // Listing it is not the restriction; reading it is. A caller that already
    // knows the URI must be refused too, or the filter above is decoration.
    if (!resourceAllowed(uri, allowedTools)) {
      throw new McpError(ErrorCode.MethodNotFound, unavailableResourceMessage(uri, allowedTools!));
    }
